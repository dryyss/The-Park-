...
import { acceptExchange, acceptExchangeWithItems, cancelExchange, proposeExchange, markExchangeAwaitingShipment, completeExchange } from "@/server/exchange/exchange.mutations";
...
export async function completeExchangeAction(exchangeId: string): Promise<ExchangeActionResult> {
  const viewer = await getAuthenticatedViewer();
  if (!viewer) return { ok: false, error: "UNAUTHORIZED" };
