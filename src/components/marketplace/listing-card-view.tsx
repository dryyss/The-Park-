...
export type ListingCardLabels = {
  minCondition: string;
  wantedBadge: string;
  ownListingBadge: string;
  ownListingYou: string;
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-2xl border border-charbon-500 bg-charbon-800 transition hover:border-charbon-400">
      <Link href={`/marketplace/carte/${l.slug}`} className="group relative block px-3.5 pt-3.5">
        ...
        >
          {l.isWant ? `${labels.minCondition} ${conditionLabel}` : conditionLabel}
        </span>
        {l.isWant && (
        ...
        <div className="mt-auto flex flex-col gap-2">
          <div className="flex items-center justify-between gap-2">
            ...
            {isOwnListing ? (
              ...
              >
                {l.isWant ? labels.actionPropose : labels.actionContact}
              </ContactSellerButton>
            )}
          </div>
