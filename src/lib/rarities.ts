 ...
 * - Rattachée à une collection : ses initiales priment ("CHR-03").
 * - Sinon, code série de la saison : "MF-03".
...
export function cardNumberLabel(
  ...
  rarityCode: string,
  seasonCode?: string,
  opts?: { seriesCode?: string | null; setSeriesCode?: string | null },
): string {
  const n = String(number).padStart(2, "0");
  const series = opts?.setSeriesCode?.trim() || opts?.seriesCode?.trim();
  if (series) return `${series}-${n}`;
